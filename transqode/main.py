...
def create_profile(p: ProfileIn):
    ...
    pid = db.execute(
        ...
        (p.name, p.video_codec, p.preset, p.quality_mode, p.icq, p.vmaf_target,
         p.audio_codec, p.audio_kbps_per_channel, p.container, p.extra_video_args, ts, ts))
...
def update_profile(profile_id: int, p: ProfileIn):
    ...
    db.execute(
        ...
        " vmaf_target=?, audio_codec=?, audio_kbps_per_channel=?, container=?,"
        " extra_video_args=?, updated_at=? WHERE id=?",
        (p.name, p.video_codec, p.preset, p.quality_mode, p.icq, p.vmaf_target,
         p.audio_codec, p.audio_kbps_per_channel, p.container, p.extra_video_args,
         db.now(), profile_id))
