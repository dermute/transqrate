...
def find_icq(input_path: Path, profile: dict, settings: dict, info: dict,
             ...
             on_cmd=lambda cmd: None,
             on_spawn=lambda proc: None) -> SearchResult:
    ...
    try:
        samples = _extract_samples(input_path, workdir, duration, sample_s, n,
                                   log, cancel_check, on_cmd, on_spawn)
        if not samples:
        ...
        def evaluate(q: int) -> tuple[float, float]:
            ...
            for i, sample in enumerate(samples):
                ...
                on_cmd(cmd)
                proc = media.run_quiet(cmd, on_spawn=on_spawn)
                if cancel_check():
                    raise Cancelled()
                ...
                score = _vmaf_score(enc, sample, ref_filter, model, on_cmd, on_spawn)
                if cancel_check():
                    raise Cancelled()
...
def _extract_samples(input_path: Path, workdir: Path, duration: float,
                     ...
                     on_cmd=lambda cmd: None,
                     on_spawn=lambda proc: None) -> list[Path]:
    samples = []
...
def _vmaf_score(distorted: Path, reference: Path,
                ...
                model: str | None = None,
                on_cmd=lambda cmd: None,
                on_spawn=lambda proc: None) -> float:
